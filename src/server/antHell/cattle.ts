...
const spawnTickDB = new worldDB<any>("spawnTick");
const growTick = 3000;
...
world.beforeEvents.playerInteractWithEntity.subscribe((ev)=>{
    const feed: string[] = ["minecraft:carrot","minecraft:seed","minecraft:wheat"];

    if(!cattles.includes(ev.target.typeId)) return;
    if(!feed.includes(ev.itemStack.typeId)) return;

    if((system.currentTick - spawnTickDB.get(ev.target)) < growTick) ev.cancel = true;
})